...
class User
{
    ...
    {
        if(isset($this->userIdentityMap[$id])){
            return $this->userIdentityMap[$id];
        }
        foreach ($this->getDataFromSource(['id' => $id]) as $user) {
            return $this->userIdentityMap[$id] = $this->createUser($user);
        }
    ...
    public function getByLogin(string $login): ?Entity\User
    {
        foreach ($this->getDataFromSource(['login' => $login]) as $user) {
            if(isset($this->userIdentityMap[$user['id']])){
                return $this->userIdentityMap[$user['id']];
            }
            if ($user['login'] === $login) {
                return $this->userIdentityMap[$user['id']] = $this->createUser($user);
            }
        }
